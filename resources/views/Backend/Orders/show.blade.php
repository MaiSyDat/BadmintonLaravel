...
@section('content')
    <div class="container py-5">
        ...
        <div class="mb-4">
            <div class="row">
                <div class="col-md-8">
                    <p><strong>Khách:</strong> {{ $order->user->name }} | {{ $order->user->phone }}</p>
                ...
                <div class="col-md-4">
                    ...
                    <p>
                        ...
                        <span class="badge text-white bg-{{ $order->status_payment ? 'success' : 'warning' }}">
                    ...
                    <p>
                        ...
                        <span class="text-white badge bg-{{ $order->status_transport ? 'info' : 'secondary' }}">
        ...
        <h5 class="mb-3">Cập nhật trạng thái giao hàng</h5>
        <form method="POST" action="{{ route('ordersManager.updateTransport', $order->id) }}">
            @csrf
            <div class="form-group">
                <select name="status_transport" class="form-control w-25">
                    <option value="0" {{ $order->status_transport == 0 ? 'selected' : '' }}>Chờ giao</option>
                    <option value="1" {{ $order->status_transport == 1 ? 'selected' : '' }}>Đã giao</option>
                </select>
            </div>
            <button type="submit" class="btn btn-primary">Cập nhật</button>
            <a href="{{ route('ordersManager.index') }}" class="btn btn-secondary">Quay lại</a>
        </form>
    </div>
@endsection
